...
//Workflow
import { AddCourseToStep,DeleteCourseFromStep } from "../../Actions/pilotAction"
...
export default class StepCard extends React.Component {   
  ...
  componentWillMount(){
    ...
    var newcourse = courses.map((course,j)=>{
        if(course.courseid == lesson.course_id){
  ...
  componentDidMount()
  {
    ...
    ondrop(event) {
      ...
      console.log("I'm a little confused",courseTitle,workflowid,stepSequence,courseid);
  ...
  render(){
                    ...
                    >
      {
        this.state.course.map((course,i) => {
            return <li key={i}>{course.courseid+" "+course.title}<Icon type="delete" id = {course.courseid }style={{float:"right"}} onClick={this.Delete.bind(this)}/></li>
      })}
      </Card>           
     	);
	}
